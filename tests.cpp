...
// Returns the sum of the damage of several monsters
// Theta(n)
...
// Display the monsters of a solution as well as the damage
// inflicted by these monsters and the use of both spells.
// Theta(n)
...
// Returns true if solution_a_tester is a valid solution for the instance (monsters, red_magic, blue_magic).
// A valid solution does not have to be optimal.
// Theta(n log n)
...
std::vector<Monstre> forceBrute(const std::vector<Monstre>& monstres, unsigned int magie_rouge, unsigned int magie_bleue) {
  ...
  for (unsigned int mask = 0; mask < (1 << monstres.size()); mask++) {
    ...
    for (unsigned int j = 0; j < monstres.size(); j++) {
      if ((1 << j) & mask) {
	solution_courante.push_back(monstres[j]); // current_solution
      }
    }
    // Theta(n log n)
